...
// h-sorted
    for (int iter = h; iter < num_size; ++iter)
      for (int iter_min = iter;
           iter_min >= h && num[iter_min] < num[iter_min - h];
           iter_min -= h)
...
void _merge(std::vector<int> &num, int low, int middle, int high) {
  ...
  for (; low <= ret_middle && middle <= high;) {
    if (num[low] <= num[middle]) ret.push_back(num[low++]);
    else ret.push_back(num[middle++]);
  }
...
// 统计重要逆序对
  for (int iter_low = low, iter_high = middle + 1;
       iter_low <= middle;
       ++iter_low) {
    while (iter_high <= high && num[iter_low] > num[iter_high] * 2LL)
      ++iter_high;
...
void _swim(std::vector<int> &num, int low, int high) {
  ...
  for (; high / 2 >= low && num[high / 2] < num[high]; high /= 2) {
    int iter_change = 0;
    if (high / 2 == 0) { iter_change = high / 2 - 1; }
    else iter_change = (high - 1) / 2;
...
void test_sort() {
  ...
  std::cout << "the inverse pairs: "
            << tiny_sort::inverse_pairs(num)
            << std::endl;
  std::cout << "the inverse pairs: "
            << tiny_sort::inverse_pairs_important(num)
            << std::endl;
  ...
  std::vector<int> vec_merge_lh = num, vec_merge_hl = num, vec_quick = num,
      vec_heap_sink = num, vec_heap_swim = num;
  tiny_sort::merge_high_to_low(vec_merge_hl,
                               0,
                               static_cast<int>(num.size()) - 1);
  ...
  std::cout << "   select sort: "
            << std::is_sorted(ret_select.begin(), ret_select.end())
            << std::endl;
  std::cout << "   insert sort: "
            << std::is_sorted(ret_insert.begin(), ret_insert.end())
            << std::endl;
  std::cout << "   shell sort: "
            << std::is_sorted(ret_shell.begin(), ret_shell.end())
            << std::endl;
  std::cout << "   merge_hl sort: "
            << std::is_sorted(vec_merge_hl.begin(), vec_merge_hl.end())
            << std::endl;
  std::cout << "   merge_lh sort: "
            << std::is_sorted(vec_merge_lh.begin(), vec_merge_lh.end())
            << std::endl;
  std::cout << "   quick sort: "
            << std::is_sorted(vec_quick.begin(), vec_quick.end())
            << std::endl;
  std::cout << "   heap sort_sink: "
            << std::is_sorted(vec_heap_sink.begin(), vec_heap_sink.end())
            << std::endl;
  std::cout << "   midian number: "
            << tiny_sort::midian_finder(num)
            << std::endl;
  std::cout << "   select the 6th number: "
            << tiny_sort::select_index(num, 6)
            << std::endl;
}
} // namespace of test
} // namespace of tiny_sort

namespace tiny_search {
// return the val index
int binary_search(const std::vector<int> &num,
                  const int &val,
                  int low,
                  int high) {
  ...
  int cmp = num[middle] - val;

  if (cmp > 0) return binary_search(num, val, low, middle - 1);
  else if (cmp < 0) return binary_search(num, val, middle + 1, high);
  else return middle;
}
...
class rb_avl_tree_base {
 public:
  ...
  explicit rb_avl_tree_base(int value = 0,
                            rb_avl_tree_base *p = nullptr,
                            rb_avl_tree_base *l = nullptr,
                            rb_avl_tree_base *r = nullptr,
...
void rb_tree::_left_rotate(rb_avl_tree_base *node) {
  ...
  if (node->parent == nullptr) _root = tmp;
...
void rb_tree::insert(int val) {
  auto new_node =
      new rb_avl_tree_base(val, nullptr, nullptr, nullptr,
                           _rb_tree_color::_red);
  insert(new_node);
}
...
// 寻找插入位置
  while (iter != nullptr) {
    p = iter;
    if (node->val < iter->val) iter = iter->left;
...
void rb_tree::_insert_fixup(rb_avl_tree_base *node) {
  while (node->parent != nullptr
      && node->parent->color == _rb_tree_color::_red) {
...
void rb_tree::delete_node(int val) {
  delete_node(search(val));
}
...
void rb_tree::_delete_transplant(rb_avl_tree_base *node, rb_avl_tree_base
*new_node) {
  if (node->parent == nullptr) _root = new_node;
  else if (node == node->parent->left) node->parent->left = new_node;
...
rb_avl_tree_base *rb_tree::search(int value) {
  ...
  while (iter != nullptr) {
    ...
    else if (iter->val > value) iter = iter->left;
    else iter = iter->right;
  }
  return nullptr;
}
...
void rb_tree::in_order() {
  _in_order(_root);
}

void rb_tree::post_order() {
  _post_order(_root);
}
...
void rb_tree::_pre_order(rb_avl_tree_base *node) const {
  if (node == nullptr) return;
  std::cout << "  " << node->val << " | "
            << ((node->color == _rb_tree_color::_red) ? "red" : "black")
            << std::endl;
...
void rb_tree::bfs() {
  ...
  while (!stack.empty()) {
    ...
    std::cout << "  " << stack.front()->val << " | "
              << (stack.front()->color == _rb_tree_color::_red ? "red"
                                                               : "black")
              << std::endl;
    stack.pop();
  }
...
class avl_tree_base {
 public:
  ...
  explicit avl_tree_base(int v = 0,
                         int bf = 0,
                         avl_tree_base *p = nullptr,
                         avl_tree_base *l = nullptr,
                         avl_tree_base *r = nullptr)
...
avl_tree_base *tree_avl::search_val(int val) {
  ...
  while (iter != nullptr) {
    ...
    else if (iter->val > val) iter = iter->left;
    else return iter;
  }
  return iter;
}
...
} // namespace of test_new_feature
} // namespace of feature_ranges

namespace test {
void test() {
  std::vector<std::vector<int>> a{{1, 2, 3}, {2, 3, 4}};
  for (auto &iter_row : a)
    for (auto &iter_col : iter_row)
      std::cout << iter_col << " ";
  std::cout << std::endl;
}
} // namespace of test
